...
void FileMetaData::newFiles(QStringList list)
{
	...
	while (i.hasNext())
	{
		...
		for (int i = 0; i < numBlocks; ++i)
		{
			...
			array.append(QCA::Hash("md5").hashToString(qba));
		}
		metadata m;
		m.fileName = str;
		m.fileSize = file.size();
		m.blocklist = array;
		m.fileId = QByteArray().append(QCA::Hash("md5").hashToString(array));
		files.append(m);
...
{
	QCA::Initializer qcainit;
	return QByteArray().append(QCA::Hash("md5").hashToString(array));
}
